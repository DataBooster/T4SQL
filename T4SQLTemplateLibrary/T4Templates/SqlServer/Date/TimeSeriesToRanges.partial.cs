...
namespace T4SQL.SqlServer.Date
{
	...
	public partial class TimeSeriesToRanges : ITemplate, ITemplateProperties
	{
		...
		public TemplateSpec GetPropertiesSpec()
		{
			...
			spec.AddProperty("KeyColumns", "COL1, COL2", null, "The key column or a comma-separated list of key columns - exclude the date column");
		...
		public IEnumerable<string> AttribColumns
		{
			get
			{
				string attrCols = this.GetPropertyValue("AttribColumns");

				if (string.IsNullOrWhiteSpace(attrCols))
					attrCols = "*";
				else
					attrCols = attrCols.Trim();

				IEnumerable<string> attribColumns = (attrCols == "*") ?
					Context.DbServerEnv.ListTableColumns(SourceView) : attrCols.SplitToCollection();

				return attribColumns.Except(new string[] { DateColumn, RangeStartDateColumn, RangeEndDateColumn },
					StringComparer.OrdinalIgnoreCase).Except(GetKeyColumns(), StringComparer.OrdinalIgnoreCase);
			}
		}
